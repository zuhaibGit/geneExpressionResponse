...
######################################
setwd("/home/zuhaib/Desktop/covid19Research/geneResponseApp/geneExpressionResponse/theApp/data_GSE148729_Calu3_totalRNA/DE")
...
write.table(s1, "long_GSE148729_Calu3_sarsCov1.txt", row.names = F, col.names = T, quote = F, sep = "\t")
...
write.table(s2, "long_GSE148729_Calu3_sarsCov2.txt", row.names = F, col.names = T, quote = F, sep = "\t")


##### BUILDING THE MOCK MATRIX #####
### 4H VS 24h DATA ###
mock.4h.mock.24h <- data.frame(Gene = row.names(data), T1 = 4, T2 = 24)
mock.4h.mock.24h <- merge(mock.4h.mock.24h, dfs$`mock-4h-mock-24h.txt`, all = T)    # This file was named wrong. It should've been mock-4h-mock-14h.txt
...
write.table(mock.4h.mock.24h, "long_GSE148729_Calu3_mockInfection.txt", row.names = F, col.names = T, quote = F, sep = "\t")
